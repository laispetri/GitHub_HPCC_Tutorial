...
3. In RStudio, using command line. [LP: this works!!!]
    ...
    - Type `git add README.md` # ctrl + alt+ enter to add code line to Terminal 
        - LP try: `git add Tutorial.Rmd` does not work. need to specify the folder
        - LP try2: `git add Tutorial/Tutorial.Rmd` works
    
    - Type `git commit -m 'changed a few notes on the Tutorial file'`
...
- Go to one of the development nodes. I use either `dev-intel16` or `dev-intel18`. Go for whichever has lower usage in parentheses. 
  - LP: I could not find the usage in parentheses
...
## Navigate to your mapped drive [LP: this does not work for me, I cannot map my drive]
...
## Laís random notes
I can run basically all the code from here using the Terminal from OnDemand
I can pull and push code as well. It is just harder to push changes given that I cannot open the RStudio there without the OnDemand (I don't know hwo to change file content in the Terminal, and might not need to do so)
I can also send jobs through the Terminal following the steps in this tutorial